...
class TopoDataset(Dataset):
    def __init__(self, theta, load, compliance):
        self.theta = theta
        self.load = load
    ...
    def __getitem__(self, idx):
        return self.theta[idx], self.load[idx], self.compliance[idx]

dataset = TopoDataset(X_theta, X_load, y_norm)
...
class SurrogateModel(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(32*15*7 + 1, 128),                 # +1 for x_load
            nn.ReLU(),
            nn.Linear(128, 1)
        )
...
for epoch in range(100):
    model.train()
    train_loss = 0.
    for theta, x_load, compliance in train_loader:
        optimizer.zero_grad()
        pred = model(theta, x_load)
    ...
    test_loss = 0.
    with torch.no_grad():
        for theta, x_load, compliance in test_loader:
            pred = model(theta, x_load)
...
torch.save({'model': model.state_dict(), 'y_mean': y_mean, 'y_std': y_std},
